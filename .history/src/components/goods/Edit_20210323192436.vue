			...
			<el-form :model="editForm" :rules="editFormRules" ref="editFormRef" label-width="70px" label-position="top">
				<el-tabs v-model="activeIndex" tab-position="left">
					...
					<el-tab-pane label="商品图片" name="3">
						<el-upload action="uploadURL" :on-preview="handlePreview" :file-list="imgList"
...
<script>
	export default {
		...
		methods: {
			//获取商品信息
			async getGoodsInfo() {
				await this.$http.get(`goods/${this.$route.params.id}`).then(response => {
			...
			postEdit() {
				this.$refs.editFormRef.validate(async valid => {
					if (valid) {
						...
						await this.$http.put(`goods/${this.editForm.goods_id}`, form).then(response => {
							const res = response.data
							if (res.meta.status === 200) {
